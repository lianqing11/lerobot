#!/usr/bin/env bash
# =============================================================================
# train_advantage_head.sh — 训练 KAI-0 风格 Advantage Head
#
# 当前默认预设：
...
#
# 【本地用法】
#   ./train_advantage_head.sh <dataset_list.txt> [选项...]
#
# 【火山云用法】控制台「自定义任务」选 PyTorchDDP，入口命令：
#   bash /path/to/train_advantage_head.sh <dataset_list.txt> [选项...]
#
# 选项:
...
#   --output_dir=DIR            输出目录，默认 ckpt/advantage_head_<dataset>_<timestamp>
...
if [[ -n "${output_dir}" ]]; then
  ...
  output_dir="ckpt/advantage_head_${dataset_label}_${timestamp}"
fi

# WandB run name 默认格式：advantage_head_<dataset>_<时间戳>
if [[ -z "${wandb_run_name}" && -n "${wandb_project}" ]]; then
  wandb_run_name="advantage_head_${dataset_label}_${timestamp}"
fi
...
if [[ "${node_rank}" -eq 0 ]]; then
  echo
  echo "╔══════════════════════════════════════════════════════════════╗"
  echo "║           Advantage Head Training Configuration              ║"
  echo "╚══════════════════════════════════════════════════════════════╝"
...
# =============================================================================
# 构建 train_advantage_function.py 参数
# =============================================================================
train_args=(
  "${script_dir}/train_advantage_function.py"
  --dataset_list_file="${dataset_list_file}"
  --backbone_family="${backbone_family}"
  --pretrained_model_name="${pretrained_model_name}"
  --vlm_variant="${vlm_variant}"
...
# 启动训练
# =============================================================================
echo "[节点 ${node_rank}] 启动 Advantage Head 训练: ${num_machines} 节点 × ${num_gpus_local} GPU = ${num_processes} 总进程"
echo
